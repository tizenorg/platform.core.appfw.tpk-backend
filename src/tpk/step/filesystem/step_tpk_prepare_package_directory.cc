...
const std::vector<std::string> kSymlinkContentEntries = {
  "res"
};
...
ci::Step::Status StepTpkPreparePackageDirectory::PrepareLinks() {
  ...
  for (auto& link_content_entry : kSymlinkContentEntries) {
    bs::error_code error;
    bf::path directory = context_->pkg_path.get() / link_content_entry;
    bf::create_directories(directory, error);
    if (error) {
      LOG(ERROR) << "Failed to create directory: " << directory;
      return Status::APP_DIR_ERROR;
    }
    for (bf::directory_iterator iter(mount_point / link_content_entry);
        iter != bf::directory_iterator(); ++iter) {
      std::string entry =
          link_content_entry + "/" + iter->path().filename().string();
      LOG(DEBUG) << "Symlink: " << entry;
      auto status = PrepareLink(entry, mount_point);
      if (status != Status::OK)
        return status;
    }
  }
